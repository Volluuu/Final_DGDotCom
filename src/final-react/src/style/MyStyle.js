...
const MyStyle = () => {
  ...
  const getStyleListSelectByTag = async (tag) => {
    ...
    const res = await axios.get(
      process.env.REACT_APP_URL + `/style/list/tags?tag=${subTag}`
    );
      ...
      <div>
        <StyledTabList>
          <div>
            ...
            <PopAndNew
              onClick={activePop}
              className={isActive.pop ? "active" : ""}
            >
              인기
            </PopAndNew>
            {sessionStorage.loginok === undefined ? (
              <AddStyleButton to={"/mystyle"} onClick={moveToForm}>
                +
              </AddStyleButton>
            ) : (
