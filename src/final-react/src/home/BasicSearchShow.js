    ...
    return (
        <div style={{width: "700px", height: "100%"}}>
            ...
            <div style={{marginBottom: "20px"}}>
                ...
                {
                    // Link로 돌리면 될듯 ㅋㅋ ez
                    recommend.map((elt, idx) => <RecLink key={idx} to={`/product/list/?keyword=${elt}`}>{elt}</RecLink>)
                }
            </div>
            <Title_p>인기 검색어</Title_p>
            <div style={{marginBottom: "20px", width: "700px", display: "flex", justifyContent: "space-around"}}>
                {/*//인기 검색어 테이블을 만들어야 할듯 없으면 테이블에 추가, 있으면 해당 검색어의 count를 1추가*/}
                <ul style={{width: "350px", justifyContent: "left"}}>
                    {
            ...
            </CategoryElt><CategoryElt to={`/product/list/?currentPage=1&keyword=패션잡화`}>
            ...
            <div style={{width:"700px", height:"180px", display:"flex", flexWrap:"wrap"}}>
                {
                    hot_brand.map((elt, idx) => <PopLink key={idx} to={`product/list/?currentPage=1&brands=${elt.brand}`}>{elt.brand}</PopLink>)
                }
            </div>
        </div>
    );
